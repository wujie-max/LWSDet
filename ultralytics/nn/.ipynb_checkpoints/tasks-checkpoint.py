...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, CrossDetect)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x,
                     RepC3, C2fCIB, mobileone_C2f, C2f_CA, C2f_DynamicConv,
                     C2f_FasterBlock, C2fCIB_S, GatedCNNBlock, C2f_GCL):
                args.insert(2, n)  # number of repeats
                n = 1

        elif m is starnet_s1:
            m = m(*args)
            c2 = m.width_list  # 返回通道列表 [24, 40, 112, 960]
            # print("width_list", c2)
            backbone = True
            ...
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
        elif m in {AIFI, EMA, SCA, LSKA, CoordAtt, BRA, ELA, SEBlock, ECLA}:
            args = [ch[f], *args]
            c2 = args[0]
        ...
        # 添加SDI结构
            ...
            c2 = args[0]
            args = [[ch[x] for x in f],args[0]]
            # c2 = int(sum(ch[x] for x in f))//2
            # c2 = max([ch[x] for x in f])

        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, CrossDetect}:
        ...
        m.np = sum(x.numel() for x in m_.parameters())  # number params  计算参数量
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect, CrossDetect)):
                return "detect"
